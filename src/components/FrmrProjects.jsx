...
const FrmrProjects = (props) => {
  ...
  const [isLoad, setIsLoad] = React.useState({
    ...
    const ProjectCreate = async ()=>{
      if(
        ...
        const res = await fetch(
          'http://localhost:3000/api/add/project_create',
          postData
        )
  ...
  const [projects, setProjects] = React.useState(undefined)
    ...
    const fetchProjects = async ()=>{
      ...
      const res = await fetch(
        'http://localhost:3000/api/get/get_projects',
        postData
      )
      const response = await res.json()
                ...
                <div className="fp-project-container">
                  <Suspense>
                  {
                    projects?.map((project)=>{
                      console.log(project)
                      return (
                        <Link onClick={()=> setIsLoad(ex=>({...ex, full: true}))} href={`farmer-dashboard/project-details/${project?.id}`}>
