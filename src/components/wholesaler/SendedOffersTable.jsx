...
const columns = [
    ...
    { id: 'trader_name', label: 'Trader', align: 'center', minWidth: 100, },
...
const SendedOffersTable = (props) => {
    ...
    const fetchOffers = async ()=>{
        ...
        '/api/users/wholesaler/get/sended_offers_list',
        postData
        )
    ...
    const proceedToBilling = async (row) =>{
        ...
        const res = await fetch(
            '/api/users/wholesaler/add/add_orders',
            postData
        )
        const response = await res.json()
        if(response.status == 200){
            router.push(`/users/wholesaler-dashboard/billing/${response?.order_id}`)
        }
