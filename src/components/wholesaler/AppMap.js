...
function AppMap({ info }) {
  ...
  async function calculateRoute() {
    ...
    setCost(parseFloat(results.routes[0].legs[0].distance.text.split(' ')[0]) * transportCost[info.transportInfo.type] + ' taka')

    // setting the infos transportCost[info.transportInfo.type]
    info.setTransportInfo(ex => ({
      ...ex,
      from: originRef.current.value,
      to: destiantionRef.current.value,
      distance: results.routes[0].legs[0].distance.text,
      cost: parseInt(results.routes[0].legs[0].distance.text.split(' ')[0]) * transportCost[info.transportInfo.type] + ' taka'
    }))
  }
