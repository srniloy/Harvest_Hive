...
const SellingTable = (props) => {
    ...
    const handleChangeRowsPerPage1 = (event) => {
        ...
        const getTotalSales = ()=>{
            ...
            props.total.setTotal(ex=>({
                ...
                total_profit: totalSales - parseInt(props.total.total.total_costs)
            }))
    ...
    const fetchSales = async ()=>{
        ...
        '/api/users/trader/get/get_Trader_sales',
        postData
        )
        const response = await res.json()
        setSalesData(response.data)
        props.salesInfo?.setSlotSalesInfo(
            response.data
        )
    ...
    const getOffersList = async (row)=>{
        ...
        const res = await fetch(
        '/api/users/wholesaler/get/get_sales_offer_list',
        postData
        )
        const response = await res.json()
    ...
    const salesAndOfferStatusUpdate = async()=>{
        ...
        const res = await fetch(
            '/api/users/wholesaler/update/update_sales_offers_status',
            postData
        )
        const response = await res.json()
        setSalesData(undefined)
